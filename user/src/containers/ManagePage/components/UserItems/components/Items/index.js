...
import { removePlaylistItem, setPlaylistRating, hideUserItem } from '../../../../../../actions/index'
        ...
        <MaterialTable
          ...
          data={query =>
            new Promise((resolve, reject) => {
              let url = `${process.env.REACT_APP_SERVER_URL}/user/video/getUserVideo/${user_id}/${playlist_id}`;
              url += `/${query.pageSize}`;
              url += `/${(query.page + 1)}`;
              fetch(url)
          ...
          actions={[
            rowData =>({
              icon: () => (
                <img src={ rowData.dotted === 1  ? images.playlistControlsDotOn : images.playlistControlsDotOff } className={classes.badgeImage} alt="dotBtn" /> 
              ),
              tooltip: rowData.dotted === 1 ? 'Un-Dot this item' : 'Dot this item',
              onClick: (event, rowData) => this.setUserItemDot(rowData)
            }),
            rowData =>({
              icon: () => (
                <img src={ rowData.hidden === 1  ? images.playlistControlsHideOn : images.playlistControlsHideOff} className={classes.badgeImage} alt="hideBtn" />
              ),
              tooltip: rowData.hidden === 1 ? 'Make this item as unhidden' : 'Make this item as hidden',
              onClick: (event, rowData) => this.setUserItemHidden(rowData)
            }),
            {
              icon: () => (
                <img src={images.interface_link} className={classes.badgeImage} alt="interface_link" />
              ),
              tooltip: 'Go to the link',
              onClick: (event, rowData) => window.open(rowData.interface_link, 'name')
            },
            {
              icon: () => (
                <RemoveIcon />
              ),
              tooltip: 'Remove this video item',
              onClick: (event, rowData) => this.removeItem(rowData.id)
            },
            {
              icon: () => (
                <AddToPhotosIcon />
              ),
              tooltip: 'Import a new video',
              isFreeAction: true,
              onClick: (event) => alert('This feature is in development. You can currently  import content using the the viewer mode')
            }
          ]}
        />
        <p className={classes.subtitle}>M-2-3</p>
      </React.Fragment>
    )
  }
};
...
const mapDispatchToProps = (dispatch) => ({
  ...
  hideUserItem: bindActionCreators(hideUserItem, dispatch),
})

export default withStyles(styles)(connect(null, mapDispatchToProps)(UserItems));