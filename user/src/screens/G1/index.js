...
class G1 extends Component {
  ...
  loadPlayListFromDB = () => {
    ...
    const { gridsets, user, selectPlaylist, showPlaylist, playlists, playlist, playlist_lock_icon } = this.state;
  ...
  getTwitchVideo = (playlist, callback) => {
    let twitchVideos = [];
    if(playlist[1] === null || playlist[1]['videos'].length === 0) {
      callback(twitchVideos);
    }
    else if(playlist[1].videos[0]['type'] === 'twitch_live')
    {
      twitchVideos.push(playlist[1]['videos'][0].url);
      callback(twitchVideos);
    }
    else {
      callback(twitchVideos);
...
//      this.addLog("Unfeatured video on cube " + (index + 1));
  ...
  renderAudioCube = () => {
    ...
    //   <div className="AdCube Cube" id={`CubeBox_4`}>
    //     <div className="MainBar">
    //       <div name="iCube5" class="VideoBox">
            
    //         <div class="WinCube" id="iCube5" style={{background:'#057ace'}}>
    //           AUDIO CUBE
    //         </div>
    //       </div>
    //       <div className="Tim" id={`Tim7`}><img alt='button' src={images.timer} width="6" height="144" id={`C15_TIM`} /> </div>
    //     </div>
    //     <div class="ButtonBar" id="C4_BUT">
    //       <img alt='button' className="imgBtn" width="50px" src={images.markAudioCubeAudio} onClick={() => { }} />
    //       <img alt='button' className="imgBtn" width="60px" src={images.btnAudioCubeAudioOff} onClick={() => { }} />
    //       <img alt='button' className="imgBtn" width="80px" src={images.btnAudioCubeLockedAudioOff} onClick={() => { }} />
    //       <img alt='button' className="imgBtn" width="70px" src={images.btnAudioCubePlaylistOff} onClick={() => { }} />
    //     </div>
  ...
  renderAdCube = (index) => {
    ...
    //         <div class="WinCube" id="iCube5" style={{background:'#43e243'}}>
    //           AD CUBE
    //         </div>
    //       </div>
    //       <div className="Tim" id={`Tim8`}><img alt='button' src={images.timer} width="6" height="144" id={`C16_TIM`} /> </div>
    //     </div>
    //     <div class="ButtonBar" id="C4_BUT">
    //       <img alt='button' className="imgBtn" width="72px" src={images.btnAdCubeSponsored} onClick={() => { }} />
    //       <img alt='button' className="imgBtn" width="90px" src={images.btnAdCubeFullScreenOff} onClick={() => { }} />
    //       <img alt='button' className="imgBtn" width="90px" src={images.btnAdCubeSubscribeOff} onClick={() => { }} />
    //     </div>
    //   </div>
    // );
...
//    document.getElementsByClassName(".pl_content_part .pl_titleThumb img").style.height="73px";
  ...
  render() {
    const { activeVideo, showPlaylist, playlist, playlists, video, gridsets, items, playlist_id, importContent, searchContent, wikiContent, dot_filter, hide_filter, openPlaylist, previewPlaylists_id, gspn, title, user, selectPlaylist, gangedGridset,  pl_view } = this.state;
...
//      console.log(item);
    ...
    SysOverFlowClass += ' ' + this.state.settingsHeight;
...
export default withRouter(connect(null, mapDispatchToProps)(G1));
