...
router.post('/checkUpdate', (req, res) => {
  ...
  if(currentEmail) {
        ...
        return res.status(400).send({ message: 'Database Connection Failed!'});
			...
			conn.query(`SELECT DISTINCT(playlist_id) FROM user_playlists WHERE user_id = '${user_id}' AND playlist_auto_update = true`, (err, rows) => {
				if (err) {
          return res.status(500).send({ message: 'Internal server error ...'});
				...
				if(rows.length === 0) {
					...
					conn.query(`DELETE FROM user_info WHERE user_id = ${user_id} AND playlist_id IN (${playlists.map((playlist) => `${playlist}`).join(', ')})`, (err) => {
						if (err) {
							return res.status(500).send({ message: 'Internal server error ...'});
						}
						conn.query(`SELECT playlist_data.playlist_id, playlist_data.video_order, playlist_data.video_id
												FROM playlist_data 
												JOIN videos on playlist_data.video_id = videos.id
												WHERE videos.dead = 0 AND playlist_data.playlist_id IN (${playlists.map((playlist) => `${playlist}`).join(', ')}) ORDER BY playlist_id, video_order`, (err, rows) => {
							if (err) {
								return res.status(500).send({ message: 'Internal server error ...'});
							}
							if(rows.length === 0) {
								...
								const data = rows.map(({playlist_id, video_order, video_id}) => {
									return [user_id, playlist_id, video_id, video_order];
								})
								conn.query('INSERT INTO user_info(user_id, playlist_id, video_id, video_order) VALUES ?', [data], (err) => {
									if (err) {
										return res.status(500).send({ message: 'Internal server error ...'});
									}
									res.send({
...
router.get('/getUserPublicPlaylists/:userId/:gridset_id', (req, res) => {
	const { userId, gridset_id } = req.params;
	const currentEmail = checkToken(req);
  if(currentEmail) {
		req.getConnection((err, conn) => {
			if (err) {
				return res.sendStatus(400).send({ message: 'Database Connection Failed!'});
			}
			getUserPublicPlaylists(conn, userId, gridset_id)
			.then((playlists) => {
        res.send({
          playlists
        });
...
router.get('/getUserPreviewPlaylists/:userId/:gridset_id/:playlist_id', (req, res) => {
	const currentEmail = checkToken(req);
	const { userId, gridset_id, playlist_id } = req.params;
  ...
  if(currentEmail) {
		req.getConnection((err, conn) => {
			...
			checkGridsetGanged(conn, userId, gridset_id)
			.then((ganged_gridset) => {
				getPreviewPlaylists(conn, userId, gridset_id, ganged_gridset)
				.then((previewPlaylists) => {
					getPreviewItems(conn, userId, previewPlaylists, playlist_id, dotfilter, hidefilter, searchfilter)
					.then((previewItems) => {
						res.send({
							success: true,
							previewItems
...
router.post('/moveUserPlaylist/:userId', (req, res) => {
	const { userId } = req.params;
	const { original_group_id, group_id, playlist_id, move_option } = req.body;
  ...
  if(email) {
        ...
        return res.status(400).send({ message: 'Database Connection Failed!'});
			...
			conn.query(`SELECT * FROM user_playlists WHERE user_id = '${userId}' AND gridset_id = '${group_id}' AND playlist_id = '${playlist_id}'`, (err, rows) => {
				if (err) {
          return res.status(500).send({ message: 'Internal server error ...'});
				}
				if(rows.length) {
					res.send({
						success : false,
						message: 'The Group already has got that playlist'
					})
				}
				else {
					conn.query(`SELECT * FROM user_playlists WHERE user_id = '${userId}' AND gridset_id = '${group_id}' ORDER BY playlist_order`, (err, rows) => {
						if (err) {
							return res.status(500).send({ message: 'Internal server error ...'});
						}
						const counts = rows.length;
						const last_order = counts > 0 ? rows[counts - 1].playlist_order + 1 : 0;
						conn.query('INSERT INTO user_playlists(user_id, gridset_id, playlist_id, playlist_order) VALUES(?, ?, ?, ?)', [userId, group_id, playlist_id, last_order], (err) => {
							if (err) {
								return res.status(500).send({ message: 'Internal server error ...'});
							}
							if(move_option === 'move') {
								conn.query(`SELECT playlist_order FROM user_playlists WHERE user_id = '${userId}' AND gridset_id = '${original_group_id}' AND playlist_id = '${playlist_id}'`, (err, rows) => {
									if (err) {
										return res.status(500).send({ message: 'Internal server error ...'});
									}
									const origin_order = rows[0]['playlist_order'];
									conn.query(`DELETE FROM user_playlists WHERE user_id = '${userId}' AND gridset_id = '${original_group_id}' AND playlist_id = '${playlist_id}'`, (err) => {
										if (err) {
											return res.status(500).send({ message: 'Internal server error ...'});
										}
										conn.query(`UPDATE user_playlists SET playlist_order =
																CASE
																WHEN playlist_order > ${origin_order} THEN playlist_order - 1
																ELSE playlist_order
																END
																WHERE user_id = '${userId}' AND gridset_id = '${original_group_id}'
										`, (err, rows) => {
											if (err) {
												return res.status(500).send({ message: 'Internal server error ...'});
											}
      ...
      return res.sendStatus(400);
		...
		conn.query(`SELECT * FROM playlists WHERE title = '${title}'`,(err, rows) => {
			...
			if(rows.length > 0) {
				...
				conn.query(`UPDATE playlists SET title = '${title}', description = '${description}' WHERE id = '${playlist_id}'`, (err) => {
					if (err) {
						return res.sendStatus(400);
					}
					res.send({
